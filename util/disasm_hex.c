...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        printf("Usage: %s <hexfile> [lstfile]\n", argv[0]);
    ...
    while (dict_addr < addr && entry_count < 100) { // Safety limit
        ...
        if (dict_addr > 0x2000 && link == 0) break; // End of dictionary
        
        if (dict_addr < addr - 1) {
                uint16_t link = memory[dict_addr];
                uint8_t name_len = memory[dict_addr + 1] & 0xFF;  // Low byte of next word
                ...
                // Calculate where code starts: after link(1 word) + name_len(1 byte) + name + padding
                // Total header size in bytes: 2 + 1 + name_len + padding
                // Padding to make total bytes even (word aligned)
                uint16_t header_bytes = 2 + 1 + name_len;
                if (header_bytes & 1) header_bytes++; // Add padding if odd
                ...
                printf("  '%s' @ 0x%04X: code at word 0x%04X (byte 0x%04X), link=0x%04X\n", 
